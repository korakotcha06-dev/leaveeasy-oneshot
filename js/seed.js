...
// ข้อความที่ใช้ตอนกฎปฏิเสธ แยกออกมาเป็นฟังก์ชันเพราะยาวและต้องอ่านรู้เรื่อง
// โครงของข้อความคือ อาการ · ทำไมถึงถูกต้อง · แล้วต้องไปทำที่ไหนแทน
function ข้อความเมื่อกฎปฏิเสธ() {
  return [
    "กฎความปลอดภัยปฏิเสธการเขียน ซึ่งถูกต้องแล้วและไม่ใช่ข้อผิดพลาดของระบบ",
    "ข้อมูลตัวอย่างมีโปรไฟล์บทบาท manager กับ hr ใบลาของคนอื่น",
    "และใบลาที่อนุมัติมาแล้วตั้งแต่สร้าง ซึ่งไม่มีผู้ใช้บทบาทใดเขียนได้เลย แม้แต่ hr",
    "ถ้าต้องการข้อมูลตัวอย่างบนฐานข้อมูลจริง ให้ใส่ผ่าน Firebase Console ในฐานะเจ้าของโปรเจกต์",
    "ถ้าต้องการไว้ทดลองหรือถ่ายคลิป ให้เปิดฐานจำลองที่พอร์ต 5050 แล้วใส่ข้อมูลผ่าน tests/fixtures",
    "รายละเอียดอยู่ใน BACKLOG.md ข้อ 3"
  ].join(" · ");
}
...
ปุ่ม.addEventListener("click", async () => {
  ...
  try {
    ...
    // เขียนไม่สำเร็จต้องบอกทั้ง "เกิดอะไรขึ้น" และ "ควรทำอย่างไรต่อ"
    //
    // บนฐานข้อมูลจริงจะเจอ permission-denied แทบทุกครั้ง และ "นี่ไม่ใช่บั๊ก"
    // ข้อมูลตัวอย่างตามสเปกหัวข้อ 7 มีโปรไฟล์บทบาท manager กับ hr
    // ใบลาของคนอื่น และใบที่อนุมัติมาแล้วตั้งแต่สร้าง ซึ่ง firestore.rules
    // ปฏิเสธทั้งสามอย่างกับผู้ใช้ทุกบทบาท รวมถึง hr เองด้วย
    // (จงใจไม่ผ่อนกฎเพื่อรองรับหน้านี้ เหตุผลเต็มอยู่ใน BACKLOG.md ข้อ 3)
    //
    // describeError() คืนข้อความกลางว่า "ไม่มีสิทธิ์เข้าถึงข้อมูลนี้" ซึ่งบอกแค่อาการ
    // หน้านี้จึงเขียนข้อความเฉพาะของตัวเองทับ เพื่อไม่ให้คนกดเข้าใจผิดว่าระบบพัง
    ช่องสถานะ.textContent = "เขียนข้อมูลไม่สำเร็จ";
    const ถูกกฎปฏิเสธ = err && err.code === "permission-denied";
    showError(ช่องผิดพลาด, ถูกกฎปฏิเสธ ? ข้อความเมื่อกฎปฏิเสธ() : describeError(err));
